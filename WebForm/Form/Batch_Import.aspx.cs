...
{
    public partial class Batch_Import : basePage
    {
        ...
        protected void btnExcel_Import_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Excel轉DataSet
                DataSet ds = EPPlus.Excel2DataSet(file);
                ...
                foreach (DataTable dt in ds.Tables)
                    objEmployeeBiz.InsertBySqlBulkCopy(dt);
        ...
        protected DataTable CSV2DataTable(string filePath)
        {
            ...
            dt.Columns.Remove("JoiningDate");

            dt.Columns[dt.Columns.Count - 1].ColumnName = "JoiningDate";
        ...
        protected DataTable Telegram2DataTable(string filePath)
        {
            ...
            using (StreamReader reader = new StreamReader(filePath))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    // 取出各欄資料
                    dataRow["Employee_ID"] = Encoding.GetEncoding("Big5").GetString(b, 0, 6).Trim();
                    dataRow["Employee_Name"] = Encoding.GetEncoding("Big5").GetString(b, 6, 50).Trim();
                    dataRow["Sex"] = Encoding.GetEncoding("Big5").GetString(b, 56, 1).Trim();
                    dataRow["Email"] = Encoding.GetEncoding("Big5").GetString(b, 57, 80).Trim();
                    dataRow["Dept"] = Encoding.GetEncoding("Big5").GetString(b, 137, 50).Trim();
                    dataRow["JoiningDate"] = Encoding.GetEncoding("Big5").GetString(b, 187, 23).Trim();
